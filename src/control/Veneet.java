...
public class Veneet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(dao.lisaaVene(vene)){
			out.println("{\"response\":1}");  //Veneen lis‰‰minen onnistui
		}else{
			out.println("{\"response\":0}");  //Veneen lis‰‰minen ep‰onnistui
		}		
	}
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(dao.muutaVene(vene)) {
			out.println("{\"response\":1}");  //Tietojen p‰ivitys onnistui	
		}else {
			out.println("{\"response\":0}");  //Tietojen p‰ivitys ep‰onnistui	
		} 		
	}
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(dao.poistaVene(tunnus)){
			out.println("{\"response\":1}"); //Veneen poisto onnistui
		}else {
			out.println("{\"response\":0}"); //Veneen poisto ep‰onnistui
		}		
	}
